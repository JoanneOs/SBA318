...
// Get a single driver by ID (render the page with driver details using EJS)
router.get('/:id', (req, res) => {
  fs.readFile(driverDataPath, 'utf8', (err, data) => {
    if (err) return res.status(500).send('Server error');
    ...
    res.render('driver', { driver }); // Render the EJS page with driver data
  });
});
